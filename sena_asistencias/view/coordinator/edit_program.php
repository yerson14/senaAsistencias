...
$id = $_GET['id'];
$programa = $programaModel->obtenerProgramaPorId($id);
...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-8">
        ...
        <form action="../../controllers/CoordinatorController.php?action=update_program&id=<?php echo $id; ?>" method="POST">
    <div class="mb-4">
        <label for="nombre" class="block text-sm font-medium text-gray-700">Nombre del Programa</label>
        <input type="text" name="nombre" id="nombre" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="<?php echo $programa['nombre']; ?>" required>
    </div>
    <div class="mb-4">
        ...
        <select name="centro_id" id="centro_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
            ...
            <?php foreach ($centros as $centro): ?>
                <option value="<?php echo $centro['id']; ?>" <?php echo $centro['id'] == $programa['centro_id'] ? 'selected' : ''; ?>><?php echo $centro['nombre']; ?></option>
            <?php endforeach; ?>
        </select>
    </div>
